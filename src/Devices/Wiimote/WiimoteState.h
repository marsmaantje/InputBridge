...
// what the rest of InputBridge (visualizer, protocol field mapper) reads -
// never the raw report bytes.
#pragma once
...
// DeviceManager to prune Wiimotes gone silent - SDL_hid has no disconnect
// callback, so staleness is the only signal available.
...
// raw and a best-effort "g" value using the nominal 0g=512/1g=~640
...
// format 0x03. Fret/strum/whammy mapping follows the community-standard
// convention (WiimoteLib, wiiuse, GH3/GHWT PC drivers); verify against
// wiibrew.org/wiki/Wiimote/Extension_Controllers/Guitar_Hero_(Wii)_Guitars
// before shipping if exact fidelity matters.
struct GuitarHeroState {
    ...
    // verification against real GHWT Drums data).
    bool drum_kick = false;
    std::array<uint8_t, 6> drum_velocity{}; // red, yellow, blue, green, orange, hi-hat
};

// Wii Balance Board. Weight in kilograms via 3-point (0/17/34 kg) linear
// interpolation against the board's own calibration data (wiibrew.org/wiki/
// Wii_Balance_Board#Calibration_Data).
...
struct BalanceBoardCalibration {
    // True only once a calibration block has been parsed AND its CRC32
    // verified (see ParseBalanceBoardCalibration()). A failed CRC leaves
...
// (WiiBrew "Wii Motion Plus"). Yaw/pitch/roll are angular *rates* (deg/s),
// not absolute orientation - integrate over time if you need an angle.
// `slow_*` marks which axes were captured in the "slow"/high-precision
// gyro range (~440 deg/s full-scale) vs. "fast" (~2000 deg/s), needed to
// pick the right zero-offset/scale when converting.
...
inline BatteryBars ClassifyWiimoteBattery(uint8_t raw) {
    // Thresholds from WiiBrew's Balance Board page; same scale used by the
    // Wiimote's own 0x20 status report battery byte.
