...
void InputExclusiveMode::SetEnabled(bool enabled) { m_Enabled = enabled; }
...
void InputExclusiveMode::ApplyLinux(SDL_Joystick *joystick) {
    ...
    if (strncmp(path, "/dev/hidraw", 11) == 0) {
        ...
        snprintf(sys_device_path, sizeof(sys_device_path), "/sys/class/hidraw/hidraw%d/device", hidraw_num);

        try {
            std::filesystem::path device_path = std::filesystem::read_symlink(sys_device_path);
            SDL_Log("HID device path: %s", device_path.c_str());

            std::filesystem::path abs_device_path = std::filesystem::canonical(std::filesystem::path(sys_device_path).parent_path() / device_path);

            SDL_Log("Absolute device path: %s", abs_device_path.c_str());

            std::vector<std::filesystem::path> search_paths = {abs_device_path / "input", abs_device_path};
...
void InputExclusiveMode::ApplyWindows(SDL_Joystick *joystick) {
    ...
    if (m_Enabled) {
        ...
        HRESULT hr = DirectInput8Create(GetModuleHandle(nullptr), DIRECTINPUT_VERSION, IID_IDirectInput8, (LPVOID *)&di_interface, nullptr);
        ...
        hr = di_device->SetCooperativeLevel(hwnd, DISCL_EXCLUSIVE | DISCL_FOREGROUND);
