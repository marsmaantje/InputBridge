...
void FlightStickVisualizer::Draw(const DeviceState &dev) {
    ...
    if (dev.num_axes >= 2) {
        ...
        draw_list->AddLine(
            ImVec2(stick_center.x - stick_radius, stick_center.y),
            ImVec2(stick_center.x + stick_radius, stick_center.y),
            IM_COL32(150, 150, 150, 100));
        draw_list->AddLine(
            ImVec2(stick_center.x, stick_center.y - stick_radius),
            ImVec2(stick_center.x, stick_center.y + stick_radius),
            IM_COL32(150, 150, 150, 100));
    ...
    if (dev.num_axes > 2) {
        ...
        // Throttle often -32768..32767. We map -32768 to 0% (bottom) and 32767
        // to 100% (top)
    ...
    if (dev.num_axes > 3) {
        ...
        draw_list->AddRect(ImVec2(p.x + x_off, p.y + y_off),
                           ImVec2(p.x + x_off + w, p.y + y_off + h),
                           color_outline);

        // Draw a bar from center
        float bar_h = (h / 2) * norm;
        if (bar_h > 0)
            draw_list->AddRectFilled(ImVec2(p.x + x_off, center.y),
                                     ImVec2(p.x + x_off + w, center.y + bar_h),
                                     color_active);
        else
            draw_list->AddRectFilled(ImVec2(p.x + x_off, center.y + bar_h),
                                     ImVec2(p.x + x_off + w, center.y),
                                     color_active);
